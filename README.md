...
각 소스에 **신뢰도 등급**을 부여해 기사 카드와 필터에 배지로 표시합니다. (`lib/sources.ts`의 `tier`)

| 등급 | 점수 | 소스 |
|------|:----:|------|
| 🟢 공식 기관 | 5 | WHO · CDC · NIH (정부·국제 보건기구의 1차 공식 발표) |
| 🟣 동료심사 학술 | 5 | PubMed (동료심사를 거친 학술 논문) |
| 🔵 통신사 | 4 | Reuters (검증 체계를 갖춘 국제 통신사) |
| ⚪ 전문 매체·포털 | 3 | Medical Xpress · Google News (2차 보도) |

- `/news` 상단에 등급 **범례**를 표시하고, 소스 필터는 **신뢰도 높은 순**으로 정렬됩니다.
- 등급 메타데이터(`TIERS`)와 조회 헬퍼(`getSourceTier`)는 `lib/sources.ts`에 정의되어 있어 새 소스 추가 시 `tier`만 지정하면 됩니다.
