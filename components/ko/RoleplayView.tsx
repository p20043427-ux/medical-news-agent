...
const ALL_B_LINES = Array.from(
  new Set(KO_CONVERSATIONS.flatMap((c) => c.lines.filter((l) => l.speaker === "B").map((l) => l.ko))),
);
...
export default function KoRoleplayView({ lang }: { lang: UiLang }) {
  const [convo, setConvo] = useState<KoConversation | null>(null);
  const { data, record } = useRoleplay("ko-roleplay");

  if (!convo) {
    return (
      <div className="px-4 pb-28 pt-3">
        <h1 className="mb-1 text-2xl font-extrabold" style={{ color: "var(--text-1)" }}>{tt(lang, "회화 롤플레이", "会話ロールプレイ")}</h1>
        <p className="mb-5 text-sm" style={{ color: "var(--text-3)" }}>{tt(lang, "당신은 ", "あなたは ")}<b>B</b>{tt(lang, " 역할이에요. 상황에 맞는 한국어를 골라 대화를 완성하세요.", " 役です。場面に合った韓国語を選んで会話を完成させましょう。")}</p>
  ...
  return <Runner key={convo.id} convo={convo} lang={lang} onExit={() => setConvo(null)} onComplete={(m) => record(convo.id, m)} />;
}

function Runner({ convo, lang, onExit, onComplete }: { convo: KoConversation; lang: UiLang; onExit: () => void; onComplete: (mistakes: number) => void }) {
  ...
  function advanceNpc() { const l = lines[revealed]; if (l) speakKo(l.ko); setRevealed((r) => r + 1); }
  function pick(opt: string) {
    if (!cur) return;
    if (opt === cur.ko) { speakKo(opt); setWrong(null); setRevealed((r) => r + 1); }
          ...
          return (
            <div key={i} className={`flex flex-col ${mine ? "items-end" : "items-start"}`}>
              <button onClick={() => speakKo(l.ko)}
                ...
                <p className="mt-0.5 text-[11px]" style={{ color: mine ? "rgba(255,255,255,.8)" : "var(--text-3)" }}>{l.ja}</p>
              </button>
            </div>
          );
        })}
      ...
      <div className="mt-4">
        {done ? (
          <div className="rounded-3xl p-6 text-center shadow-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <p className="mt-2 text-lg font-extrabold" style={{ color: "var(--text-1)" }}>{mistakes === 0 ? tt(lang, "완벽해요!", "完璧です！") : tt(lang, "대화 완료!", "会話完了！")}</p>
            <p className="mt-1 text-sm" style={{ color: "var(--text-3)" }}>{tt(lang, `틀린 선택 ${mistakes}회`, `間違い ${mistakes}回`)}</p>
            <div className="mt-4 grid gap-2">
              <Button variant="brand" size="free" onClick={() => { setRevealed(0); setMistakes(0); setWrong(null); }} className="py-3" style={{ background: "#2563EB" }}>{tt(lang, "다시 하기", "もう一度")}</Button>
          ...
          <div className="rounded-3xl p-5 shadow-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <p className="mt-1 text-base font-bold" style={{ color: "var(--text-1)" }}>{cur.ko}</p>
            <p className="mt-0.5 text-sm" style={{ color: "var(--text-3)" }}>{cur.ja}</p>
            <Button variant="brand" size="free" onClick={advanceNpc} className="mt-3 w-full py-3" style={{ background: "#2563EB" }}>{tt(lang, "▶ 들려주고 다음", "▶ 聞いて次へ")}</Button>
          </div>
        ) : cur ? (
          <div>
            <p className="mb-2 px-1 text-sm font-bold" style={{ color: "var(--text-2)" }}>{tt(lang, "이렇게 말해보세요: ", "こう言ってみましょう: ")}<span style={{ color: "var(--text-1)" }}>{cur.ja}</span></p>
              ...
              {options.map((o) => {
                ...
                return (
                  <button key={o} onClick={() => pick(o)} disabled={isWrong}
                    ...
                    style={isWrong
                      ? { borderColor: "#EF4444", background: "#EF444412", color: "var(--text-3)" }
                      : { borderColor: "var(--border)", background: "var(--card)", color: "var(--text-1)" }}>
                    {o}{isWrong && <span className="ml-2 text-xs" style={{ color: "#EF4444" }}>{tt(lang, "다시 골라보세요", "もう一度選んでください")}</span>}
                  </button>
                );
              })}
            </div>
