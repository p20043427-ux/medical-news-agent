  ...
  if (!convo) {
    return (
      <div className="px-4 pb-28 pt-3">
        ...
        <p className="mb-5 text-sm" style={{ color: "var(--text-3)" }}>{tt(lang, "당신은 ", "あなたは ")}<b>B</b>{tt(lang, " 역할이에요. 상황에 맞는 일본어를 골라 대화를 완성하세요.", " 役です。場面に合った日本語を選んで会話を完成させましょう。")}</p>
        ...
        {done ? (
          <div className="rounded-3xl p-6 text-center shadow-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <div className="mt-4 grid gap-2">
              <Button variant="brand" size="free" onClick={() => { setRevealed(0); setMistakes(0); setWrong(null); }} className="py-3">{tt(lang, "다시 하기", "もう一度")}</Button>
          ...
          <div className="rounded-3xl p-5 shadow-sm" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            ...
            <Button variant="brand" size="free" onClick={advanceNpc} className="mt-3 w-full py-3">{tt(lang, "▶ 들려주고 다음", "▶ 聞いて次へ")}</Button>
          </div>
        ) : cur ? (
          <div>
