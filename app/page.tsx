...
function RootInner() {
  ...
  useEffect(() => {
    ...
    if (saved === "jp" || saved === "en" || saved === "ko" || saved === "travel") setLang(saved);
          ...
          <div className="relative">
            <div className="mb-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/70">Japan Travel</p>
              </div>
            </div>
            <div className="mb-4 flex flex-wrap gap-2">
              {["여행 회화", "관광지", "교통", "음식"].map((tag) => (
                <span key={tag} className="rounded-full bg-white/20 px-2.5 py-1 text-xs font-semibold text-white">{tag}</span>
              ))}
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm text-white/80">일본 여행 필수 가이드</p>
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20 text-white">→</span>
            </div>
          </div>
        </button>
